...
def add_recipe(username, title, content, type="manual"):
    safe_execute("INSERT INTO recipes (username,title,content,type) VALUES (%s,%s,%s,%s)",
                 (username,title,content,type))

def get_recipes(username):
    if not cursor: return []
    cur = safe_execute("SELECT id,username,title,content,type FROM recipes WHERE username=%s", (username,))
    return [{"id": r[0],"username":r[1],"title":r[2],"content":r[3],"type":r[4]} for r in cur.fetchall()] if cur else []
