...
async def main_page():
    ...
    with ui.row().classes("mx-auto mt-6"):
        ...
        ui.button("Change Password", on_click=lambda: ui.navigate.to("/change_password")).classes("bg-yellow-500 text-white")
        async def logout():
            await clear_jwt()
            ui.navigate.to("/login")
        ...
        for r in backend.get_recipes(username):
            with ui.card().classes("w-full mb-2 p-3"):
                ...
                with ui.row():
                    ...
                    ui.button("Delete", on_click=delete_recipe).classes("bg-red-500 text-white")
...
async def edit_recipe(title: str):
    ...
    if not recipe:
        ui.label("Recipe not found").classes("text-red-500"); return
...
async def change_password_page():
    ...
    with ui.card().classes("w-96 mx-auto mt-20 p-6 shadow-lg"):
        ...
        async def change():
            success = backend.change_password(username, new_password.value)
            if success:
                ui.notify("Password changed successfully", color="green")
                ui.navigate.to("/")
            else:
                ui.notify("Superuser password cannot be changed", color="red")
        ...
        for u in users:
            with ui.card().classes("w-full mb-2 p-3"):
                ...
                if not u.get("approved"):
                    def approve_user_action(uname=u["username"]):
                    ...
                    with ui.row():
                        def change_password_popup(uname=u["username"]):
                            with ui.dialog() as dialog, ui.card():
                                ...
                                pwd_input = ui.input("New Password", password=True).classes("w-full")
                        ...
                        def delete_user_action(uname=u["username"]):
...
# Run App
# -------------------------
ui.run(title="Recipe Manager", port=8080, reload=False, storage_secret="super_secret_session_key_123")
